...
def create_profile_for_user(
    ...
    # 1️⃣ Existing: create local Profile row
    connection.execute(Profile.__table__.insert(), {"user_id": target.id})

    # 2️⃣ NEW: also insert into Supabase public.user_profiles
    connection.execute(
        text("""
            INSERT INTO public.user_profiles (id, full_name, dietary_preferences, allergies)
            VALUES (:id::uuid, :full_name, '', '')
